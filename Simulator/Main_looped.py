...
#program has risk of going into infinite loop
#terminate and restart if program does not execute in 5 minutes for 100 simulations
...
def pitching_change():
	...
	for x in range(10): # Generate blank stats for box score
		print(pitchers_used[pitching_team(half_inning)][-1])
...
backup_array=[batters,pitchers,relief_pitchers,closers]
for i in range(num_sims):
	...
	on_base = [-1, -1, -1, -1]
	earned_runs = 0
	batters=backup_array[0]
	pitchers=backup_array[1]
	relief_pitchers=backup_array[2]
	closers=backup_array[3]
	...
	while gameover == False:  # Main game loop
		...
		if (
			...
			# relief_pitchers=backup_array[3]
...
home_prob=home_counter/num_sims
away_prob=away_counter/num_sims
print("End of simulation. \033[1;93;40m")
print(teams["home"] + " Simulation Win Probability:\033[0m " + str(home_prob))
print(teams["away"] + " Simulation Win Probability:\033[0m " + str(away_prob))
...
num_games=home_pd[(home_pd["Tm"]==abbrs["home"]) & (home_pd["Opp"]==abbrs["away"])].shape[0]
num_home_wins=home_pd[(home_pd["Tm"]==abbrs["home"]) & (home_pd["Opp"]==abbrs["away"])].loc[home_pd["W/L"].str.contains("W",case=True)].shape[0]
if num_games!=0:
	actual_home_prob=num_home_wins/num_games
	actual_away_prob=1-actual_home_prob
	print(teams["home"] + " Actual Win Probability:\033[0m " + str(actual_home_prob) + " After " +str(num_games)+ " Matches in " +str(years["home"]) )
	print(teams["away"] + " Actual Win Probability:\033[0m " + str(actual_away_prob) + " After " +str(num_games)+ " Matches in " +str(years["home"]) )
else:
	print("The teams did not play against each other in the "+years["home"]+" season.")
